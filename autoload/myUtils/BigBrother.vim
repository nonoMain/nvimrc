...
" function, color is \"#<html color value>\"
function! myUtils#BigBrother#GetClosetTermColor(color)
	let l:best_color = 0
...
" BG: background color
function! myUtils#BigBrother#HighlightDict(key, dict)
...
" Gets number of a buffer
" Returns a dict with the next keywords:
" ErrorCount   - amount of signs displayed in the buffer that matched with s:ErrorCount group
" WarningCount - amount of signs displayed in the buffer that matched with s:WarningCount group
" HintCount    - amount of signs displayed in the buffer that matched with s:HintCount group
...
function! myUtils#BigBrother#GetWinGitBranch()
